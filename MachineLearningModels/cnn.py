...
class ConvolutionalNeuralNetwork(Model):
    ...
    def add(self, type='Conv2D', layer, input_dim=None, kernel_initializer=None, activation=None):
        if (type == 'Conv2D'):
            self.model.add(Conv2D(layer, input_dim=input_dim, kernel_initializer=kernel_initializer, activation=activation))
        elif (type == 'Flatten'):
            self.model.add(Flatten(layer, input_dim=input_dim, kernel_initializer=kernel_initializer, activation=activation))
        elif (type == 'Dense'):
